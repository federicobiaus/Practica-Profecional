...
*/
$grid->addColumn('id_eva', 'ID', 'integer', NULL, false); 
$grid->addColumn('id_materia', 'Codigo Carrea', 'integer'); 
$grid->addColumn('materia', 'Materia', 'string');  
$grid->addColumn('fecha_examen', 'Fecha', 'date'); 
$grid->addColumn('tipo_examen', 'Tipo', 'string');  
$grid->addColumn('action', 'Action', 'html', NULL, false, 'id_eva');  
...
*/
$mydb_tablename = (isset($_GET['db_tablename'])) ? stripslashes($_GET['db_tablename']) : 'evaluaciones_planificacion';
                                                                       
$result = $mysqli->query('SELECT *, date_format(fecha_examen, "%d/%m/%Y") as fecha_examen FROM '.$mydb_tablename );
$mysqli->close();
